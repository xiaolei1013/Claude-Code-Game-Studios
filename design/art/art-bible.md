...
### Dungeon Combat Presence

This subsection governs how the party's heroes appear *standing in the dungeon run scene*, reacting to combat. It is the art-direction half of GDD #35 (Hero Combat Presence & Animation); the precise on-screen placement, sprite sizes, and layer z-order are owned by the UX spec (`design/ux/dungeon-run-view.md`), not here. The art bible owns how the heroes *read and feel*; the UX spec owns where they sit.

**One sprite per occupied formation slot.** The dungeon scene renders exactly as many heroes as the player has formed up — a data-driven count (`formation_size`, default three but never assumed). Empty slots render nothing; there is no ghost, placeholder, or filler silhouette for an unfilled slot. A solo run shows one hero, alone and unafraid; a full formation shows the whole party. The scene must look composed at every count from one upward (see GDD #35 §E "Edge Cases").

**The party is the warmest thing on screen.** This is the visual proof of the run being *theirs*. Per Section 2's Dungeon Run lighting contract, the biome backdrop is lit cool (≈4500K) and the heroes carry their own warm key (≈2900K lantern/torch light). The heroes are rim-lit warm against the cooler dungeon so they separate instantly from both the background and the enemy line. Lighting *is* the readability system: warm = ours, cool = the dungeon. An artist must never let a hero fall into the background's color temperature — if a hero reads as "part of the scenery," the shot has failed.

**In-scene tier, silhouette-first.** Dungeon heroes are the in-scene LOD (32–48px, per "LOD Philosophy" above), not downscaled portraits. Each hero must be identifiable by silhouette and class color alone at run distance — the player should never have to squint to tell the Warrior from the Mage. Heroes must not merge with each other or with enemies; the warm rim-light plus the UX spec's spacing keeps each one a distinct, readable shape.

**Register stays cozy.** Section 2 sets the Dungeon Run energy at *Medium* — competent, calm, "watching capable friends work," not spectacle or frenzy. Even mid-combat the heroes read as in-control. This is a No-Fail-State game (Pillar 1); the dungeon is a place the party *works*, not a place they suffer. Reaction beats (below) punctuate that calm; they do not replace it. The baseline an idle hero returns to between beats is the existing in-scene idle cycle — the run breathes.

**Composites under the HD-2D pass.** Heroes sit in the sharp mid-plane of the tilt-shift depth field (ADR-0019); the painterly biome backdrop falls into the softer far-plane, and lantern motes (premultiplied-alpha additive FX, §8.2.3) may drift in front. The hero layer must never break the depth read — heroes are the in-focus subject of the diorama.

### Per-Class Action Poses (Attack / Hit / Victory)

GDD #35 ships visible hero reactions in two stages: tween-based beats first (no new art), then per-class action *frames* (the Phase-3 art spend, GDD #35 Story 011). This subsection is the authoring spec for those frames. Combat in this game is aggregate — the resolver emits discrete kill events, not per-hero attacks (ADR-0010) — so these poses are **expressive theater, not a literal strike-by-strike simulation.** Each pose must read as *that class doing its thing*, punctuating the idle, then returning to idle.

**Binding production rules for every action pose:**

- **Frame convention.** Each action pose is authored as a **4-frame in-scene strip**, identical in canvas dimensions and horizontal layout to the class's idle strip, so the existing sprite-slicing pipeline (`ClassSpriteFactory` → `SpriteSheetAnimator`, validated on Recruit cards and the Hero Detail modal) ingests it with zero new framework. Do not author 3- or 5-frame action strips — a wrong frame count silently breaks the fixed-count column slice.
- **One-shot, not looping.** Action strips play once at a faster cadence than the 6 fps idle (the exact beat durations are GDD #35 §D's domain), then the hero snaps back to the looping idle. Frame 4 must compose cleanly back into idle frame 1.
- **The "hit" recoil is personal, and distinct from the floor shake.** Section 2 already establishes that heavy hits shake the *floor* via a camera micro-offset, **not** sprite animation. The per-hero hit pose is the hero's *own* body recoil — it composes *with* the camera shake, it does not replace or duplicate it. Camera = the dungeon trembling; recoil = this hero bracing.
- **No reuse of motion — extended to actions.** The "no hero may reuse another hero's secondary idle motion" rule (above) applies in full to action poses. Each class attacks, recoils, and celebrates through the prop and body language that already define its idle. The Warrior's action is driven by the shield, the Mage's by the staff-focus light, the Rogue's by the cloak, the Cleric's by the implement glow, the Ranger's by the bow-arm, the Tactician's by the document — the same identity anchors, escalated.
- **No gore, no death (Pillar 1 / ADR-0021).** There is no killed-hero pose. The defeat beat is a *dignified slump* — see below — never a corpse. This is a No-Fail-State game; a lost run is a setback the party walks away from.
- **Format.** All frames PNG-32, sRGB, full alpha, per §8.2.1 — the same standard as idle and portrait frames. Prop-light (Mage focus, Cleric implement) is the brightest source in the frame and must bloom *warm*, never cool, to stay inside the lantern-lighting identity.

**Per-class pose catalogue.** Each entry gives the read for the three reaction beats GDD #35 fires (attack on `enemy_killed`/`boss_killed`, victory on `floor_cleared_first_time`, defeat-slump on `run_defeated`):

| Class | Attack (kill/boss beat) | Hit / recoil | Victory | Defeat slump |
|---|---|---|---|---|
| **Warrior** | Shield-led shoulder-check: weight drops onto the front foot, shield leads forward — a bulwark *advancing*, mass over flourish. | Braces behind the shield, plants the back foot, absorbs with a half-step skid. The wall does not stagger far. | Grounds the shield butt-down, stands tall, fist to chest — a guardian's salute. | Sinks to one knee behind the lowered shield, head up but weary. Still the wall, just resting. |
| **Mage** | Raises the staff; the focus-crystal flares from idle-pulse to a contained warm bloom at the staff head (no projectile leaves frame — combat is aggregate). Channeling, not throwing. | The staff-light *gutters and dims*; the Mage flinches and shields the focus with a forearm. The recoil is about the light, not the body. | Lifts the staff overhead; the focus blooms into a sustained celebratory warm glow, free hand open. The light *is* the celebration. | Staff dips, focus-light fades to a low ember, the Mage steadies on the staff like a walking-stave. The light survives, dimmed. |
| **Rogue** | Drops low and snaps forward into a precise blade-thrust; the cloak whips with the dart (idle drift escalated to a whip). Targets the gap. | Spins away, cloak flaring as a defensive veil — evasion-flavored, rolls with the blow. Never a tank's brace. | A relaxed blade-flip and re-sheathe, cloak settling, a confident half-turn. Casual mastery. | A low crouch, cloak drawn around like a shelter, one knee down. Coiled, not broken. |
| **Cleric** | Raises the implement; its glow brightens into a soft warm radial pulse — a blessing-strike, light not fire. | The implement-glow dims; the Cleric bows the head and draws the implement *to the chest* (distinct from the Mage's sideways forearm-shield). Faith under strain. | Raises the implement high; a gentle halo blooms, free hand to heart — benediction. The warmth spreads outward. | Kneels with the implement cradled at the chest, head bowed; the glow narrows to a candle-flame, steady. Sheltering the light. |
| **Ranger** | A clean nock-draw-loose in profile (idle bow-arm micro-adjust becomes a full draw); the loose and bow-hand follow-through sell the shot — no arrow tracked. | The draw *breaks*; the Ranger pivots off the back foot and re-sets stance. Recoil reads as "lost the shot, re-aiming." Footwork-driven. | Spins the bow once and rests it across the shoulders, an easy archer's stance, free hand raised. Relaxed precision. | Drops to a crouch, bow planted tip-down like a staff, scanning low. Patient, regrouping. |
| **Tactician** | Snaps the document/map up and *points forward* — does not strike. The Tactician commands; the party's aggregate damage is the plan landing. (This is why aggregate combat fits them best.) | The document crumples to the chest, a step back, a quick reassessing glance — "the plan met resistance, recalculating." | Rolls the document closed with a satisfied flourish, tucks it away, a small confident nod — "as planned." Understated. | Lowers the document, kneels, one finger still marking a line on it — already planning the next attempt. Composed. |

Every cell above is distinct in both the **body action** and the **driving prop**, satisfying the no-reuse rule across all four beats. The defeat column is uniformly *dignified, recoverable, lantern-still-lit* per Pillar 1 and ADR-0021 — the difference between classes is the *posture of resilience*, never the presence or absence of harm.

**Reduce-motion fallback (art requirement).** Under the `reduce_motion` accessibility flag, action strips do not play — each resolves to a single held frame that still *reads as the pose* (the attack's peak frame, or the slump's settled frame), never a strobe and never a blank. Authors must ensure frame 1 (idle-return) and the held action frame each stand alone as a legible single image. GDD #35 §C.8 owns the runtime behavior; this is the art constraint that makes it possible.
