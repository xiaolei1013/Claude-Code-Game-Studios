...
| **Class Synergy System** (#32, V1.0 first-pass 2026-05-09 + 2026-05-14 Triple Strike addition) | Hard — config storage | Synergy constants: `STEEL_WALL_GOLD_MULT = 1.25`, `TRIPLE_STRIKE_GOLD_MULT = 1.25` (added 2026-05-14 — 3-Rogue parallel to Steel Wall, conditional on archetype == "armored"), `TRIPLE_THREAT_GOLD_MULT = 1.15`, `ARCANE_ELITE_XP_MULT = 1.20`, `BASE_XP_PER_KILL = 10`, `class_synergy_audio_suppress_window_seconds = 2.0`. Per `class-synergy-system.md` §G. **Implementer note**: V1.0 first-pass currently stores these as `const` in `dungeon_run_orchestrator.gd` rather than `economy_config.tres` per ADR-0018 Alternative C (deferred migration). No formula change to Economy itself. AC-CS-16 enforces ≤+50% per-synergy multiplier cap via static analysis across all four synergy constants. |
