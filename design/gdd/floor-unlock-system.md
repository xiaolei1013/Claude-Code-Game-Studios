...
## K — Sprint 24 S24-N2 Re-Discovery Audit (2026-05-16)

**Status**: VERIFIED-IN-PLACE. Sprint 24 S24-N2 was scoped as "author `design/gdd/floor-unlock-system.md` per 8-section GDD template; implementation deferred to Sprint 26+." Grep before authoring (per Sprint 24 Process Rule #3 — "honest dependency status check") surfaced this existing GDD: **1250 lines**, full 8-section structure (A–H), 14 ACs (AC-FU-01..14), 14 Open Questions (I.1–I.15 excluding I.2 closed), and a comprehensive review history through Pass-9 (2026-04-21) plus a Pass-PROBE-EXECUTED empirical-falsification entry that established the "empirical probe is the only acceptable evidence for engine-idiom claims" rule (referenced in `docs/engine-reference/godot/modules/autoload.md`).

**Sprint 24 S24-N2 closure**: design-only scope already satisfied by this pre-existing GDD. No new authoring needed.

**Same pattern as S24-S2 (Onboarding GDD)**: both Sprint 24 design-only stories had pre-existing GDDs that the Sprint 24 Day-0 plan didn't grep for. The grep-first process rule fired correctly on both — without it, both would have produced duplicate GDDs that drifted from the live specs.

**What this closure does NOT do**:
- Does NOT close Open Questions I.1, I.3–I.13 (those are V1.0 / pre-implementation / post-launch concerns and remain owners' work).
- Does NOT reopen I.11 — the "FOURTH consecutive wrong engine-idiom claim" lesson (Pass-PROBE-EXECUTED 2026-04-21) is canonical and remains deferred to V1.0 multi-biome authoring.
- Does NOT trigger any implementation work. The Sprint 26+ implementation epic recommendation stands.

**Lesson reinforcement** (third occurrence in Sprint 24): GDD authoring stories must grep `design/gdd/*.md` first to verify the GDD doesn't already exist. The systems-index.md entry is the canonical signal:
- GDD #16 Floor Unlock System: existing draft authored through 9 review passes through Sprint 18.
- GDD #29 Onboarding First-Session: existing draft authored 2026-05-06 (closed in S24-S2).

**Sprint 24 retro carries this as a confirmed process learning**: "Before scoping a 'author GDD X' story, grep `design/gdd/` for the slug and check systems-index.md for the row. If the GDD exists at any draft status, the story is verification-only, not authorship. The Sprint 24 Day-0 plan failed this check on both S24-S2 and S24-N2 — establish the grep-first check as part of `/sprint-plan new` to prevent recurrence."
