...
| **Class Synergy System** (#32, V1.0 first-pass 2026-05-09 + 2026-05-14 Triple Strike addition) | Indirect (archetype string source) | Reads enemy `archetype` string ("bruiser", "armored", "skirmisher", etc.) for the two conditional synergies: Steel Wall (`archetype == "bruiser"` triggers ×1.25 — Warrior counters Bruiser) and Triple Strike (`archetype == "armored"` triggers ×1.25 — Rogue counters Armored, added 2026-05-14 for symmetric class treatment). Does NOT call the resolver directly — receives the archetype via Combat Resolution's per-kill schedule. Per `class-synergy-system.md` §C.1 + §F. | Per-kill conditional check; no formula coupling. |
