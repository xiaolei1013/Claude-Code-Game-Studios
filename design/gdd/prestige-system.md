...
Status: **design RATIFIED 2026-06-23 and ALREADY IMPLEMENTED** (see the Status update at the top of this file). Prestige is live in `src/` — `HeroRoster.prestige_hero()`, the compounding global multiplier applied to kill gold + kill XP, the Hall of Retired Heroes surface, and the V1→V2 save migration all ship in V1.0. The earlier "`prestige_multiplier = 1.0` hardcoded / future epic" framing is superseded by that shipped reality; this GDD is the design contract that shipped work follows.
