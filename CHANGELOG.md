...
- **Ships disabled (`enabled = 0.0`) by default on both scenes.** First playtest (2026-05-14) of the activated shader surfaced a "Gold: 1824" ghost-stack artifact at the top of the Guild Hall: with no biome background art yet, the BackBufferCopy captures the UI labels + buttons themselves, and the 9-tap vertical Gaussian smears them into ghost copies. This is structurally correct shader behavior on the wrong content — the fix is real background sprites underneath the UI, not a different shader. The shader ships as ready-to-deploy infrastructure per the Sprint 15 N2 warm-lantern precedent; designer flips `enabled = 1.0` per-scene when biome art lands. `test_guild_hall_tilt_shift_ships_disabled_by_default` + `test_dungeon_run_view_tilt_shift_ships_disabled_by_default` pin the disabled-by-default state so a future tuning pass cannot silently re-enable it without context.
