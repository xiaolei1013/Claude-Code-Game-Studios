...
# NOTE (Amendment 2, 2026-06-03): the original recipe had `rng.state = 0` here
# "for reproducibility". That line was a BUG — empirically it clobbers the seed,
# making the pool identical for every seed AND every counter. It has been
# REMOVED; `rng.seed = X` alone gives both reproducibility and variety.
# ... draw N class_ids from active classes per OQ-RC-2 cadence (Amendment 1: WITHOUT replacement) ...
```
...
### Amendment 1 — OQ-0015-2 resolved: draw WITHOUT replacement (2026-06-03)

**Context**: OQ-0015-2 (pool same-class deduplication) was left open for "Sprint 12+
Story 4". The §Risk table already recommended the mitigation: *"Pool-generation
algorithm SHOULD prefer same-class deduplication (e.g. 'draw without replacement
until N distinct classes')."* With the roster grown to 7 active classes and the
draft expanded to `POOL_SIZE = 4` (was 3 — a tuning bump justified by the larger
roster; layout-safe at the 1280px Steam Deck width), the prior with-replacement
draw produced a duplicate ~65% of the time, undercutting the "more meaningful
choice" intent.

**Decision**: `_regenerate_pool()` draws **without replacement** (OQ-0015-2 option
b) via a partial Fisher-Yates shuffle over the active-class list — every card in a
draft of `POOL_SIZE` is a **distinct** class. When the active-class set is smaller
than `POOL_SIZE` (content removal / sparse test env), fall back to with-replacement
so the pool still fills `POOL_SIZE` rows; the §OQ-RC-3 global-per-class cost rule
keeps duplicate slots cost-consistent in that fallback path. The draw remains
deterministic under the seeded RNG.

**Consequences**: distinct drafts; no "two warriors at the same price" confusion;
the with-replacement cost-stability behavior (AC-RC-11) is still representable and
tested via manual `_set_pool([...])` fixtures. Tested by
`recruitment_skeleton_test.gd::test_regenerate_pool_draws_distinct_classes_when_enough_available`.

### Amendment 2 — Seed-clobber bug fixed: `rng.state = 0` removed (2026-06-03)

**Context**: the §OQ-RC-1 recipe set `rng.state = 0` *after* `rng.seed = X`,
commented "explicit reset for reproducibility" (and the GP-ADR gate approved it as
"deterministic"). Empirically (Godot 4.6.1, `/applications` headless probe + a
50-seed test sweep), setting `state = 0` after `seed` **clobbers the seed**: every
`save_pool_seed` AND every `refresh_counter` produced the *identical* pool. This
silently defeated the three stated goals of OQ-RC-1 — anti-save-scum per-save
uniqueness (§Risk table), per-refresh re-roll variety (refresh was a no-op), and
cloud-save derivability. The gate's verification only checked *same*-seed →
same-sequence (which `state = 0` trivially satisfies), never *different*-seed →
*different*-sequence.

**Decision**: remove the `rng.state = 0` line. `rng.seed = (save_pool_seed ^
refresh_counter)` alone provides both reproducibility (same seed → same sequence,
per Godot's RNG contract) and the required seed/counter sensitivity.

**Consequences**: the recruit pool now genuinely varies per save and per refresh,
as designed. Persisted `_current_pool` snapshots load unchanged (no save
corruption); the new behavior takes effect on the next refresh after upgrade.
Regression guards: `recruitment_skeleton_test.gd::test_different_refresh_counter_produces_different_pool_when_classes_available`
and `::test_different_save_seed_produces_different_pool` (both assert pools are not
all-identical across a sweep — they fail under the old `state = 0` behavior).
