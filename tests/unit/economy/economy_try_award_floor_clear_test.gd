...
func test_economy_try_award_floor_clear_first_call_credits_gold_and_returns_true() -> void:
	...
	economy.first_clear_awarded.connect(func(biome_id: String, floor_index: int) -> void: first_clear_emissions.append(floor_index)
	)
	...
	assert_bool(economy._floor_clear_bonus_credited.has("forest_reach_f3")).is_true()
	assert_int(economy._floor_clear_bonus_credited.get("forest_reach_f3", -1)).is_equal(3000)
...
func test_economy_try_award_floor_clear_second_identical_call_returns_false_no_mutation() -> void:
	...
	var economy: Node = EconomyScript.new()
	economy.try_award_floor_clear("forest_reach", 3, 3000)  # prime
	...
	economy.first_clear_awarded.connect(func(_b: String, _f: int) -> void: first_clear_emit_count[0] += 1)
	...
	assert_int(economy.get_lifetime_gold_earned()).is_equal(3000)
	assert_int(economy._floor_clear_bonus_credited.get("forest_reach_f3", -1)).is_equal(3000)  # unchanged
...
func test_economy_try_award_floor_clear_win_repeat_losing_sequence_total_3000() -> void:
	...
	var result1: bool = economy.try_award_floor_clear("forest_reach", 3, 3000)
	var result2: bool = economy.try_award_floor_clear("forest_reach", 3, 3000)
	var result3: bool = economy.try_award_floor_clear("forest_reach", 3, 1500)
	...
	assert_int(economy.get_lifetime_gold_earned()).is_equal(3000)
	assert_int(economy._floor_clear_bonus_credited.get("forest_reach_f3", -1)).is_equal(3000)  # ledger unchanged after 1st
...
func test_economy_try_award_floor_clear_losing_first_credits_halved_and_emits_signal() -> void:
	...
	var result: bool = economy.try_award_floor_clear("forest_reach", 3, 1500)

	# Assert — true, gold +1500, ledger {3: 1500}, first_clear_awarded fires once
	assert_bool(result).is_true()
	assert_int(economy.get_gold_balance()).is_equal(1500)
	assert_int(economy._floor_clear_bonus_credited.get("forest_reach_f3", -1)).is_equal(1500)
	assert_int(first_clear_emissions.size()).is_equal(1)
	assert_int(first_clear_emissions[0]).is_equal(3)
...
func test_economy_try_award_floor_clear_win_after_losing_credits_delta_no_re_emit() -> void:
	...
	var economy: Node = EconomyScript.new()
	economy.try_award_floor_clear("forest_reach", 3, 1500)  # LOSING prime: gold=1500, ledger {3:1500}
...
func test_economy_try_award_floor_clear_reclaim_subsequent_calls_return_false() -> void:
	...
	var economy: Node = EconomyScript.new()
	economy.try_award_floor_clear("forest_reach", 3, 1500)  # LOSING
	economy.try_award_floor_clear("forest_reach", 3, 3000)  # WIN reclaim → total=3000
	...
	var r1: bool = economy.try_award_floor_clear("forest_reach", 3, 3000)  # exact ceiling → false
	var r2: bool = economy.try_award_floor_clear("forest_reach", 3, 1500)  # below ceiling → false
	...
	var r3: bool = economy.try_award_floor_clear("forest_reach", 3, 3001)
	assert_bool(r3).is_true()
	assert_int(economy.get_gold_balance()).is_equal(3001)  # 3000 + delta(1)
	assert_int(economy._floor_clear_bonus_credited.get("forest_reach_f3", -1)).is_equal(3001)
...
func test_economy_try_award_floor_clear_win_then_losing_losing_returns_false() -> void:
	# Arrange — WIN first
	var economy: Node = EconomyScript.new()
	economy.try_award_floor_clear("forest_reach", 3, 3000)  # WIN prime
...
func test_economy_try_award_floor_clear_win_then_zero_bonus_returns_false() -> void:
	...
	# Act
	var result: bool = economy.try_award_floor_clear("forest_reach", 3, 0)
...
func test_economy_try_award_floor_clear_floor_index_zero_returns_false_no_ledger_mutation() -> void:
	...
	var result: bool = economy.try_award_floor_clear("forest_reach", 0, 500)
	...
	assert_bool(economy._floor_clear_bonus_credited.has("forest_reach_f0")).is_false()
	assert_int(gold_emit_count[0]).is_equal(0)
	assert_int(first_clear_emit_count[0]).is_equal(0)
...
func test_economy_try_award_floor_clear_floor_index_six_returns_false_no_ledger_mutation() -> void:
	...
	var result: bool = economy.try_award_floor_clear("forest_reach", 6, 500)

	# Assert — false; no key 6 in ledger
	assert_bool(result).is_false()
	assert_int(economy.get_gold_balance()).is_equal(0)
	assert_bool(economy._floor_clear_bonus_credited.has("forest_reach_f6")).is_false()
...
func test_economy_try_award_floor_clear_negative_bonus_returns_false_no_mutation() -> void:
	...
	var result: bool = economy.try_award_floor_clear("forest_reach", 1, -100)

	# Assert — false; ledger key 1 absent; floor remains uncredited
	assert_bool(result).is_false()
	assert_int(economy.get_gold_balance()).is_equal(0)
	assert_bool(economy._floor_clear_bonus_credited.has("forest_reach_f1")).is_false()
	assert_int(gold_emit_count[0]).is_equal(0)
	assert_int(first_clear_emit_count[0]).is_equal(0)
...
func test_economy_try_award_floor_clear_negative_one_bonus_returns_false() -> void:
	...
	# Act
	var result: bool = economy.try_award_floor_clear("forest_reach", 2, -1)

	# Assert
	assert_bool(result).is_false()
	assert_bool(economy._floor_clear_bonus_credited.has("forest_reach_f2")).is_false()
...
func test_economy_try_award_floor_clear_int64_min_bonus_does_not_crash() -> void:
	...
	var result: bool = economy.try_award_floor_clear("forest_reach", 1, -9223372036854775808)
...
func test_economy_try_award_floor_clear_after_negative_bonus_valid_call_still_credits() -> void:
	...
	var economy: Node = EconomyScript.new()
	economy.try_award_floor_clear("forest_reach", 1, -100)  # rejected; floor 1 stays uncredited
	...
	assert_int(economy._floor_clear_bonus_credited.get("forest_reach_f1", -1)).is_equal(500)
	assert_int(first_clear_emissions.size()).is_equal(1)
...
func test_economy_try_award_floor_clear_zero_bonus_on_uncredited_floor_returns_false() -> void:
	...
	var result: bool = economy.try_award_floor_clear("forest_reach", 1, 0)
...
func test_economy_try_award_floor_clear_valid_call_after_zero_bonus_credits_normally() -> void:
	...
	var economy: Node = EconomyScript.new()
	economy.try_award_floor_clear("forest_reach", 1, 0)  # rejected; no insert
	assert_bool(economy._floor_clear_bonus_credited.has("forest_reach_f1")).is_false()  # verify no phantom key
...
func test_economy_try_award_floor_clear_offline_replay_flag_cleared_emits_normally() -> void:
	...
	var economy: Node = EconomyScript.new()
	economy._is_offline_replay = true
	economy.try_award_floor_clear("forest_reach", 3, 3000)  # silent credit during replay
	...
	var result: bool = economy.try_award_floor_clear("forest_reach", 4, 2500)
