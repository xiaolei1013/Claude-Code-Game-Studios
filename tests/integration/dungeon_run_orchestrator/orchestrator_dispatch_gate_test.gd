...
#     /root/FloorUnlock when no spy was pre-injected; pre-injected spy survives.
...
func _make_orch_with_spy(spy: _SpyFloorUnlock) -> Node:
	var orch: Node = OrchestratorScript.new()
	# DI must happen BEFORE add_child so the lazy-bind in _ready sees the
	# pre-injected spy and skips auto-binding.
	orch.set_floor_unlock(spy)
	add_child(orch)
	auto_free(orch)
	return orch
...
# ---------------------------------------------------------------------------
# TR-026 lazy-bind: _ready auto-binds /root/FloorUnlock when no spy injected
# ---------------------------------------------------------------------------
...
func test_tr026_pre_injected_spy_survives_ready_lazy_bind() -> void:
	# Arrange — inject a spy via set_floor_unlock BEFORE add_child
	var spy: _SpyFloorUnlock = _SpyFloorUnlock.new()

	# Act
	var orch: Node = _make_orch_with_spy(spy)

	# Assert — orchestrator's _floor_unlock is the spy, NOT the autoload
	assert_object(orch._floor_unlock).is_equal(spy)
...
func test_tr026_dispatch_with_locked_floor_emits_floor_locked_validation_failed() -> void:
	# Arrange — spy returns false for is_unlocked
	var spy: _SpyFloorUnlock = _SpyFloorUnlock.new()
	spy.unlocked_value = false
	var orch: Node = _make_orch_with_spy(spy)
	...
	assert_int(spy.last_floor_index).is_equal(3)
...
func test_tr026_dispatch_with_unlocked_floor_advances_to_active_foreground() -> void:
	# Arrange — spy returns true
	var spy: _SpyFloorUnlock = _SpyFloorUnlock.new()
	spy.unlocked_value = true
	var orch: Node = _make_orch_with_spy(spy)
	var formation: Array = ["dummy_hero_a"]
