...
|----|------|-------|-----------|--------------|---------------------|
| S12-M1 | **Economy.recruit_cost formula implementation** — closes the STUB at `economy.gd:519` (currently returns 0). Per ADR-0013 §recruit_cost: `BASE_RECRUIT_COST_PER_TIER[tier] × (1 + COPIES_OWNED_MULT × copies_owned)`. EconomyConfig.tres data-driven. | economy-designer + gameplay-programmer | 0.5 | none | Unit tests for tier 1/2 × copies 0/1/2/3; cost-stability invariant; AC-EC-RC-* per economy-system.md §H. |
| S12-M2 | **Recruitment Stories 2–4 — try_recruit transaction integration tests + refund path + pool generation** (per recruitment-system.md §J) | gameplay-programmer + qa-tester | 1.0 | S12-M1 done (real cost curve) | AC-RC-04 happy path (gold + hero + signal); AC-RC-05/06/07/08 failure-path tests against live Economy + HeroRoster; AC-RC-09 add_hero contract-violation refund test (spy roster); AC-RC-10/12 pool generation with OQ-0015-2 dedup-policy choice + pool_refreshed signal. |
| S12-M3 | **Recruitment Stories 5–8 — cost-stability + Save/Load schema migration + RecruitScreen wire-up + AC-RC-14 CI grep** (per recruitment-system.md §J) | gameplay-programmer + ui-programmer | 1.0 | S12-M2 done | AC-RC-11 cost-stability invariant test; save-schema version bump (CURRENT_SAVE_VERSION 1→2 if pool fields are renamed; otherwise additive — no bump per ADR-0015 Migration Plan); RecruitScreen instantiates + tap-to-recruit happy path; CI grep enforces AC-RC-14 single-writer. |
| S12-M4 | **OfflineProgressionEngine Stories 1–3 — autoload skeleton + boot orchestration + OfflineSummary class** (per offline-progression-engine.md §J) | gameplay-programmer + godot-gdscript-specialist | 1.0 | S11-X6 + S11-X7 ✓ (flush_offline_signals already shipped) | Autoload at rank 15; boot reads TickSystem.offline_elapsed_seconds + offline_cap_seconds; OfflineSummary class with 7 fields locked per GDD §C; emits offline_replay_started + offline_replay_completed signals. |
...
| S12-N3 | **Story 015 — perf verification suite** (Story 016 AC-7/AC-8 deferred) — `tests/performance/save_load/save_persist_perf_test.gd` + p95 benchmarks | qa-lead + gameplay-programmer | 0.5 | none |
...
| S12-N5 | **Real XP-curve formula** — replace S10-M4 stub `+1-per-clear` grant with EconomyConfig-driven curve | economy-designer + gameplay-programmer | 0.5 | none |
