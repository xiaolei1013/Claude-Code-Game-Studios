...
**Story 005 DONE** — the party heroes now render on `dungeon_run_view`: an additive `PartyDioramaLayer` (sibling on root, `MOUSE_FILTER_IGNORE`, z=1 sharp plane in front of the tilt-shift DoF) holds a centered `PartyFrontLine` HBox of 72px nearest-neighbour `TextureRect` slots, one per OCCUPIED formation slot (count **data-driven** from `HeroRoster.get_formation_heroes()`, proven by 3/2/0-hero tests — never the hardcoded 3 the dominant bug class would have produced), each showing the class idle frame 0 via `ClassSpriteFactory.get_idle_frames()` with its `class_id` stashed in the `&"hero_class_id"` meta. Verified: 5 new render-assertion tests (incl. a warrior texture-load assertion proving the sprite ACTUALLY renders, not an empty node) + all 29 pre-existing `dungeon_run_view_screen_test` cases pass — 34/34, 0 orphans.

**Now: Phase 1 — Story 006** — wire `SpriteSheetAnimator` to each hero slot via `ClassSpriteFactory.animate()` so the idle loop breathes. MUST be `_process`-driven on the per-slot `&"_IdleAnimator"` child, NEVER in `_on_tick_fired` (the 20 Hz zero-alloc hot path per ADR-0025 §C.9). The `class_id` meta each slot already carries lets Story 006 attach animators without re-reading the roster. Then Story 007 (extend the Story-012 per-tick zero-alloc budget test). Phase 3's per-class art (Story 011) + Phase 4's human playtest (Story 016) remain the external-dependency gates surfaced to the user.
