...
**Stories 005–006 DONE** — the party heroes now render AND idle-animate on `dungeon_run_view`. Story 005 added an additive `PartyDioramaLayer` (sibling on root, `MOUSE_FILTER_IGNORE`, z=1 sharp plane in front of the tilt-shift DoF) holding a centered `PartyFrontLine` HBox of 72px nearest-neighbour `TextureRect` slots, one per OCCUPIED formation slot (count **data-driven** from `HeroRoster.get_formation_heroes()`, proven by 3/2/0-hero tests — never the hardcoded 3 the dominant bug class would have produced), with each slot's `class_id` stashed in the `&"hero_class_id"` meta. Story 006 then swapped the static frame-0 set in `_make_hero_slot()` for `ClassSpriteFactory.animate(slot, class_id)`, which attaches a `_process`-driven `SpriteSheetAnimator` child `&"_IdleAnimator"` that loops the class breathing idle — structurally decoupled from `_on_tick_fired` (ADR-0025 §C.9 "two clocks, never the tick"), and a zero-per-frame no-op when art is absent. Verified: 7 cumulative diorama/animation tests (render count + warrior texture-load + animator-attached/`is_processing()` + deterministic `_process(0.2)` frame-advance) + all pre-existing `dungeon_run_view_screen_test` cases — 36/36, 0 orphans.

**Now: Phase 1 — Story 007** — low-frequency hero-state reflection + extend the Story-012 per-tick zero-alloc budget test to PROVE `_on_tick_fired` gained no allocation from the diorama (Stories 005/006 deliberately self-drive the animators on `_process`, so the hot path should be untouched — this test makes that a guarded invariant). Heroes reflect coarse run state on human-frequency signals, never per-tick. Phase 3's per-class art (Story 011) + Phase 4's human playtest (Story 016) remain the external-dependency gates surfaced to the user.
